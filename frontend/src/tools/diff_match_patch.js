...
diff_match_patch.prototype.decodeURI = function (text) {
    try {
        ...
        while (i < text.length) {
            ...
            var byte1 =
                (this.digit16(text[i + 1]) << 4) + this.digit16(text[i + 2])
            if ((byte1 & 0x80) === 0) {
            ...
            var byte2 =
                (this.digit16(text[i + 4]) << 4) + this.digit16(text[i + 5])
            if ((byte2 & 0xc0) !== 0x80) {
                throw new URIError("URI malformed")
            }
            byte2 = byte2 & 0x3f
            ...
            var byte3 =
                (this.digit16(text[i + 7]) << 4) + this.digit16(text[i + 8])
            if ((byte3 & 0xc0) !== 0x80) {
                throw new URIError("URI malformed")
            }
            byte3 = byte3 & 0x3f
...
diff_match_patch.prototype.patch_addContext_ = function (patch, text) {
    ...
    if (
        ...
        diff_match_patch.prototype.isHighSurrogate(
            text[patch.start2 + patch.length1 + padding]
        )
    ) {
        padding++
    }
